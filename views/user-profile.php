...
<main class="w-full px-4 md:px-12 lg:px-44 inline-block">
  ...
  <?php
  $db = _db();
  $q = $db->prepare('SELECT * FROM orders WHERE order_created_by = :user_id');
  $q->bindValue(':user_id', $_GET['user_id']);
  $q->execute();
  $orders = $q->fetchAll();
  ?>
  ...
  <div class="w-3/4 h-screen p-4 ml-48" id="orders">
    ...
    <p class="px-8">This is all you orders. If the delivered at is 0, your order is still on its way 🛵</p>
    ...
    <?php foreach ($orders as $order) : ?>
      ...
      <div class="flex w-full pt-4 px-8">
        ...
        <div class="w-2/3"><?= $order['order_delivered_by'] ?></div>
